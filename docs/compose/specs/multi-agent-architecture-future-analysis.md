...
- **方向**：P0 无动作（判定链路完整）；P1 修复 `update_stats` 断链，让路由成功率真正自适应；P2 判定结果结构化回传 UI（复杂度 + 路由理由），增强可解释性。
...
- **方向**：P0 工作流节点接线真实工具执行并写文件；P0 合并两个 WorkflowEngine 实例（暂停/取消真生效）；P1 DAG 生成去硬编码 dept_order，由 planner 产出真实 DAG；P1 角色产出走 artifact 模式（降低 L1 的 LLM 调用放大）；P2 检查点系统补齐自动失败检测/防重复执行 [25]。
...
- **方向**：P0 critic/grounding 诚实化（接入真实 LLM 审查，或明示为确定性规则检查）；P1 审查重心转向确定性验证（测试/lint 门禁）+ 单次 LLM 审查；P2 审查意见全程结构化（延续 `structured_feedback`），形成"审查报告 → 迭代"的可见闭环。
...
- **方向**：P1 补全技能闭环自动触发（项目结束自动审核→写增量→打包）；P1 目录/加载语义对齐 Agent Skills 标准（保持 CoW）；P2 技能市场/跨项目经验融合，把"技能会成长"做成产品的核心可感知价值。

#### 横向基础能力（跨支柱）

| 方向 | 行业参照 | 优先级 |
|------|----------|--------|
| 债务治理：删死代码（两套 discussion 死代码）、mock-sso 镜像、文档与代码对齐 | L8；会议/投票已降级为辅助机制 | P0 |
| 审批诚实化 + HITL 分级（93% 审批疲劳 [26] → 白名单 + 分级 + 分类器 [27]） | L7 | P0/P1 |
| 混合执行真正接线（roleLocations 贯穿 TS 编排器，hybrid 按工具类型路由） | L6 | P1 |
...
**优先级汇总**：P0 = 让五大支柱"名副其实"（工作流真执行、审查诚实化、债务清理、双引擎合并）；P1 = 补全主线能力（路由自适应、技能闭环、真实 DAG、artifact、混合执行、HITL 分级）；P2 = 差异化深化（Agent Skills 对齐、MCP、durable execution、技能市场）。
