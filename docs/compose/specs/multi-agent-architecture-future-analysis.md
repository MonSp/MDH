...
- P2 残余补完（2026-08-13）：补齐两项"刻意保留"的未完成项（3 commits 合并至 main@b0951d2，backend 960 全绿）——① failover 扩展到全部可达 LLM 消费点（decompose_task/handle_critical_blocker/ReviewPipeline 三站点接入 `_mark_model_failed`/`on_model_error` 回调 + 回调自身异常防护；semantic/discussion 经 `get_model_fn` 无 coordinator 可达性标注不适用）；② `persist_execution` per-execution 异步锁（async API + `asyncio.Lock` 串行化并发落盘，create 同步路径保留内部同步写）。至此 P2 无任何未完成项。
