...
| 版本 | 日期 | 主题 |
|------|------|------|
| **0.1.0.0** | 2026-08-14 | 初始版本基线（项目启动至产品定型） |
| **0.1.1.0** | 2026-08-16 | 会议纪要全链路 + 资产沉淀闭环（M3/M4/M5） |
| **0.1.2.0** | 2026-08-17 | 调研驱动 14 项全栈改进 + 技能市场三阶段 + 模型自产工作流 |
| **0.1.2.1** | 2026-08-18 | Bug fixes + Docker healthcheck + 路由模块就绪 |
| **0.1.2.2** | 2026-08-19 | TS 能力补齐（HITL/LLM 守卫/渐进加载/4 新 LLM 提供商） + server.py 端点迁移 |
| **0.1.3.0** | 2026-08-19 | Playwright 浏览器自动化（25 工具，TS + Python 双端） |
| **0.1.3.1** | 2026-08-19 | 前端架构治理（handler 拆分 5 模块 + Zustand store + 52 测试） |
| **0.1.3.2** | 2026-08-20 | 大文件拆分（16 文件 -3500 行） + code review 修复 |
| **0.1.3.3** | 2026-08-20 | 循环导入修复 + 类型安全（any 清零） |
| **0.1.3.4** | 2026-08-20 | 资产-技能协作闭环增强（人工审核/资产编辑/复用率仪表盘/双端注入） |
| **0.1.3.5** | 2026-08-20 | 规则有效性追踪 + 自动降级 + 告警 + 统计报表 + 报表导出 |
| **0.1.3.6** | 2026-08-20 | 跨团队技能共享质量门禁 + 自适应路由闭环修复 |
| **0.1.4.0** | 2026-08-20 | 数字员工职业发展核心数据层（AgentProfile + XP + 技能树 + 角色晋升） |
| **0.1.4.1** | 2026-08-20 | 职业发展前端面板（CareerPathPanel + SkillTreeView + 部门筛选） |
| **0.1.5.0** | 2026-08-21 | 路由感知技能等级 + 晋升驱动任务分配 + 真实 AI 闭环验证 |
| **0.1.5.6** | 2026-08-22 | 规则自进化（低分规则自动生成改进版） |
| **0.1.5.7** | 2026-08-22 | 联动进化（规则→资产→技能网络级联更新） |
| **0.1.5.8** | 2026-08-22 | 反思优先级队列（自驱动选择反思目标） |
| **0.1.5.9** | 2026-08-22 | 抗过拟合（多样性检查+老化+探索/利用平衡） |
| **0.1.5.10** | 2026-08-22 | 多团队进化联邦（信任评分+智能订阅+跨团队有效性） |
| **0.1.5.11** | 2026-08-22 | CI/CD 进化健康度门禁 + GitHub Actions |
| **0.1.5.12** | 2026-08-22 | 能力边界感知（置信度地图+未知领域检测） |
| **0.1.5.13** | 2026-08-22 | 系统自省（功能利用率+模块健康度+改进提案） |
| **0.1.5.14** | 2026-08-22 | 人机协作反馈回路（结构化反馈→规则转化） |
| **0.1.5.15** | 2026-08-22 | 前端协作改进（内联反馈+技能徽章+进化通知） |
| **0.1.5.16** | 2026-08-22 | 文档感知协作（文档解析+上下文注入） |
| **0.1.5.17** | 2026-08-22 | 活文档协作（代码感知+数据感知+产出物追踪） |
| **0.1.5.18** | 2026-08-22 | Agent 持久记忆（持久记忆文件+自动摘要+老化） |
| **0.1.5.19** | 2026-08-22 | 跨会话学习闭环（任务前检索+任务后写入） |
| **0.1.5.20** | 2026-08-22 | 自主交付（Git交付+通知+报告） |
| **0.1.5.21** | 2026-08-22 | Agent 自省优化（表现分析+弱项识别） |
| **0.1.5.22** | 2026-08-22 | 主动式监控（健康巡检+风险预警） |
| **0.1.5.23** | 2026-08-22 | 团队协同优化（协同分析+瓶颈识别） |
| **0.1.5.24** | 2026-08-22 | 端到端集成验证（4 条链路 8 个测试） |
| **0.1.6.3** | 2026-08-22 | API 文档与一致性（OpenAPI 标签分组 + CHANGELOG 同步） |
| **0.1.6.4** | 2026-08-22 | 性能优化与缓存（TTLCache + 热数据缓存） |
| **0.1.6.5** | 2026-08-22 | 开发者体验（Makefile + 架构文档） |
| **0.1.6.6** | 2026-08-22 | 多租户基础（租户管理 + 团队级 API key） |
| **0.1.6.7** | 2026-08-22 | 多模型支持（9 个提供商 + 模型路由 + 自动降级） |
| **0.1.6.8** | 2026-08-22 | Webhook 集成（事件注册 + 签名验证 + 投递日志） |
| **0.1.6.9** | 2026-08-22 | 集成验证 + 文档同步 |
| **0.1.6.10** | 2026-08-22 | E2E 测试强化（5 条关键路径 14 个测试） |
| **0.1.6.11** | 2026-08-22 | 错误处理标准化（标准错误码 + 静默异常修复） |
| **0.1.6.12** | 2026-08-24 | 季度路线图 18 项 + v1.6.x 审查 14 项修复 + 评测基准 + 性能优化 |
| **0.1.7.0** | 2026-08-26 | Agent OS 架构：A2A 协议基础设施 + TS Orchestrator 瘦身 + 双层状态同步 |
| **0.1.7.1** | 2026-08-26 | Claude Code A2A 适配器 + A2A 生产加固（SSRF 防护/Prometheus 指标/HTTP 复用） |
| **0.1.7.2** | 2026-08-26 | E2E A2A 测试 (31项) + 前端 A2A 管理面板 + Docker Claude Code 适配器 + AGENTS.md 架构更新 |
| **0.1.7.3** | 2026-08-26 | AGENTS.md 架构全面更新 + 前端 A2A 面板接入导航 + TS Orchestrator 清理 |
| **0.1.7.4** | 2026-08-26 | A2A WebSocket 实时推送 + Orchestrator 遗留清理 + 冒烟测试 |
| **0.1.7.5** | 2026-08-26 | A2A 任务可观测性（日志+耗时指标）+ E2E 验证增强（31→40 项） |
| **0.1.7.6** | 2026-08-26 | A2A 代码审查修复（SSRF DNS 防护/内存泄漏/阻塞修复/URL 可配置/认证绕过） |
| **0.1.7.7** | 2026-08-26 | A2A 代码审查收尾（async 回调/死连接清理/废弃 API/并发防护/差异文档化） |
| **0.1.8.0** | 2026-08-26 | 智能调度：A2A 自动路由集成（SimpleExecutor A2A 路径 + 前端执行偏好 + E2E 测试） |
| **0.1.8.1** | 2026-08-26 | A2A 经验闭环（A2APostProcessor：经验提炼 + XP 授予 + 记忆写入 + 路由统计） |
| **0.1.8.2** | 2026-08-26 | A2A 路由质量提升（阈值 0.7 + 本地执行守卫 + execution_preference 端到端传递） |
| **0.1.8.3** | 2026-08-26 | A2A 经验闭环修复（AgentProfileManager 接入 + XP 授予目标修正） |
| **0.1.8.4** | 2026-08-26 | 集成差距修复（SimpleExecutor 进化管线 + A2A API 签名修正 + Webhook 接入） |
| **0.1.8.5** | 2026-08-26 | 集成差距修复续（TeamSynergy 自动记录 + ProactiveMonitor 后台调度） |
| **0.1.8.6** | 2026-08-26 | 集成差距修复终章（AssetInjection 接入 MeetingCoordinator + CapabilityBoundary 接入任务流） |
| **0.1.8.7** | 2026-08-26 | A2A 代码审查 P1 修复（readBody 限制 + 竞态锁 + YAML→JSON + agent_id 可配置 + 重试机制） |
| **0.1.8.8** | 2026-08-26 | A2A 代码审查 P2 修复（10 项 Minor：未使用导入 + 健康持久化 + awaitable + 关键词去重 + Card 去重） |
| **0.1.8.9** | 2026-08-26 | 数据源统一：人类反馈规则写入 SQLite（消除反馈与主检索系统的数据源分裂） |
| **0.1.8.10** | 2026-08-26 | Orchestrator 全部 17 个 TypeScript 编译错误清零（8 源码 + 9 测试） |
| **0.1.9.0** | 2026-08-27 | 进化可见 + LLM 提炼 + 前端治理（EvolutionEvent 系统/A-B 证明/DeepSeek 经验提炼/进化面板顶级入口/3D 懒加载/apiClient 统一） |
| **0.2.0** | 2026-08-27 | 生产加固 + 产品体验（异常修复/限流/WS校验/快速体验/API文档/CONTRIBUTING/移动端适配） |
| **0.2.1** | 2026-08-27 | 多租户全路径强制（TenantMiddleware + 5 端点租户过滤 + 15 测试） |
| **0.2.2** | 2026-08-27 | CI 覆盖率门禁（pytest-cov 75% 最低覆盖率） |
| **0.2.3** | 2026-08-27 | Docker 生产优化（资源限制 + 日志轮转 + 健康检查 + 依赖排序） |
| **0.3.0** | 2026-08-27 | 可观测性栈（Prometheus + Grafana + 4 仪表盘 20 面板 + 7 告警规则 + 11 指标） |
| **0.3.1** | 2026-08-27 | 功能验证 + 性能基准（22 E2E 子系统测试 + 7 性能基准 + 基线 JSON） |
| **0.3.2** | 2026-08-27 | 扩展基准 + 压力测试（6 扩展基准 + 6 压力测试 + 7 回归检查） |
| **0.3.3** | 2026-08-27 | 收束修复（回归容差 + 测试警告 + 全量验证 2001 passed） |
| **0.4.0** | 2026-08-27 | 交互式引导 Onboarding（6 步向导 + 3 示例任务 + 首次自动触发 + 重播按钮） |
...
| **0.5.7** | 2026-09-16 | 技能进化与记忆模块优化（SQL检索/数据源统一/老化调度/LLM进化/去重/AB统计/记忆合并清理） |
| **0.5.8** | 2026-09-16 | 探索逻辑修复 + FTS5 + summary缓存 + 循环检测 + ABTracker质量粒度 + 惰性单例 + 前端质量分析 + 语义检索 + LLM自动审批 |
| **0.6.0** | 2026-09-16 | 自调优引擎 Phase 1+2（参数注册表 + 离线优化器，架构层 RSI 探索） |
| **0.6.1** | 2026-09-16 | 自调优引擎 Phase 3+4（影子验证 + 自动回滚，RSI 闭环完成） |
| **0.6.2** | 2026-09-16 | TuningRegistry 运行时接线（9 个参数实时生效）+ 优化器维度分析 |
| **0.6.3** | 2026-09-16 | TuningRegistry 全量接线（18/18 参数实时生效，RSI 闭环完整） |
| **0.6.4** | 2026-09-16 | AutoOptimizer 接入生产运行时（RSI 自动优化循环） |
| **0.6.5** | 2026-09-16 | RSI 监控面板（实时状态 + 手动触发 + 部署历史） |
| **0.6.6** | 2026-09-16 | 规则去重统一（所有创建路径走 _save_new_rule） |
...
| 测试 | Vitest (TS) + pytest (Python) | 1726 TS 测试用例（前端 1726 + orchestrator 216）+ 2052 Python 测试 |
