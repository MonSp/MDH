...
| **0.5.8** | 2026-08-27 | 探索逻辑修复 + FTS5 + summary缓存 + 循环检测 + ABTracker质量粒度 + 惰性单例 + 前端质量分析 + 语义检索 + LLM自动审批 |
| **0.6.0** | 2026-08-27 | 自调优引擎 Phase 1+2（参数注册表 + 离线优化器，架构层 RSI 探索） |
| **0.6.1** | 2026-08-27 | 自调优引擎 Phase 3+4（影子验证 + 自动回滚，RSI 闭环完成） |
