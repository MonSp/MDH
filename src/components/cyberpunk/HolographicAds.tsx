...
import { GlitchAd } from './GlitchText'
...
function generateAdsNearBuildings(buildings: BuildingData[]): AdData[] {
  ...
  // 为每栋建筑分配3-4个广告牌，覆盖率达到80%以上
  for (let bi = 0; bi < buildings.length; bi++) {
    const b = buildings[bi]
    // 确保每栋建筑至少3个面有广告牌（覆盖率达到75%+）
    const adCount = 3 + Math.floor(Math.abs(Math.sin(bi * 3.7)) * 2) // 3-4 ads per building
    for (let ai = 0; ai < adCount; ai++) {
      const ad = adTexts[adId % adTexts.length]
      // 选择建筑的4个面之一，确保覆盖3个面以上
...
function HolographicAd({ data }: { data: AdData }) {
  ...
  return (
    <group ref={groupRef} position={data.position} rotation={data.rotation}>
      {/* 使用 GlitchAd 组件（包含背景、文字、边框、扫描线） */}
      {data.type === 'mega_billboard' ? (
        <GlitchAd
          text={data.text}
          color={data.color}
          width={data.width}
