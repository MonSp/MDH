...
- [x] T6: Add background aging scheduler for `AgentMemory` — acceptance: `age_memories` called periodically for all agents (covers: F5)
- [x] T7: Inject KnowledgeNetwork/TeamFederation into ExperienceExtractor constructor — acceptance: no new instances inside `_evolve_rule_impl` (covers: F8)
- [x] T8: Consolidate to single ExperienceExtractor instance in server.py — acceptance: one shared instance across all consumers (covers: F8; depends: T7)
- [x] T9: Propagate `team_id` through StateSyncManager — acceptance: `prepare_task_metadata` accepts and uses team_id (covers: F10)
- [x] T10: Debounce `_generate_markdown` in AgentMemory — acceptance: markdown not rewritten on every add_memory call (covers: F6)
