...
2026 年 5 月，我们开始问一个问题：**AI agent 到底应该是什么？**

市面上的 AI 助手是「工具」——你给它一个指令，它给你一个回答，然后就忘了。下一次对话，它什么都不知道。它不会积累经验，不会成长，不会和其他 AI 协作。

但真实世界的工作不是这样的。一个团队里，有人负责前端，有人负责后端，有人审查代码，有人管理进度。每个人都在学习，都在变强。上个项目踩的坑，下个项目就不会再踩。

我们想做的事情很简单：**让数字员工从工具变成同事。**

### 骨架（v1.0，2026-05 ~ 08）

最初的三个月，我们在搭骨架。一个 3D 赛博朋克风格的虚拟办公室，agent 在里面开会、讨论、投票、执行任务。CEO 负责拆解需求，架构师负责设计，开发负责写代码，QA 负责审查。43 个技能包、25+ 角色、18 种工具。

但这时候的数字员工虽然能干活，还是「一次性」的。每次开会，它们都从零开始。没有记忆，没有成长。

### 记忆（v1.1，2026-08-16）

数字员工开始有「记忆」了。每次执行任务，产出物自动入库为「资产」。经验被提炼为技能规则。下一次执行同类任务时，这些规则会自动注入到 agent 的上下文里。

但记忆是脆弱的。规则写了，注入了，但没人知道它到底有没有用。

### 进化（v1.3，2026-08-20）

我们修复了进化闭环的致命 bug，然后给每条规则加上了有效性评分。用了 3 次以上、成功率低于 40% 的规则自动降级退回待审核。数字员工开始有了「免疫系统」——坏经验被淘汰，好经验留下来。

一个团队总结的高分经验，通过质量门禁可以跨团队共享。

### 职业（v1.4，2026-08-20）

42 个技能被组织成技能树，每个技能三个等级。10 个部门有各自独立的晋升标准。完成任务获得经验值，经验值够了就升级。高级员工做简单任务，经验值衰减——必须做真正有挑战的事才能成长。

数字员工有了自己的职业档案，跨项目持久化。它们在成长，在变强，在从初级工程师变成技术负责人。
...
这个闭环跑通了。数字员工不再是工具，而是同事。它们有自己的记忆、经验、技能，并且在持续进化。
