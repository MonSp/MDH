  ...
  return new Promise(async (resolve) => {
    ...
    if (opts.keyless) {
      ...
      for (const f of Object.values(snapshot.files)) {
        ...
        // 空文件阈值与 LLM 路径对齐（内容 ≤10 字符视为空）：keyless 按读到的内容字符数
        // （chars）判定而非字节 size——CJK 场景 5 个中文字符 = 15 UTF-8 字节，但 LLM 路径
        // 按 read_file 字符数（length=5 ≤ 10）判空，size 仅作快照元数据
        else if (isFileEmpty(f.chars)) issues.push({ type: 'quality', severity: 'high', desc: `文件为空: ${f.path}` });
      }
    ...
    const finish = async () => {
      ...
      for (const cmd of s.verifyCommands) {
        ...
        try {
          ...
          // T5 回放以 keyless 快照为准：keyless 用 `bash -o pipefail` 执行，且全部
